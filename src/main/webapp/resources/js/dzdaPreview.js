$(function(){
	...
	var data=[
		{id:1,pId:0,name:"正卷",open:true,drag:false,isParent:true,dropInner:false,path:"sfdsgdsgg"},
	...
	function onClick(event, treeId, treeNode){
		if(treeNode.level==2){
			$(".img_preview").html("").append('<img id="myImg" />');
			$("#myImg").prop("src",treeNode.path);
			/*resizeImg($(".img_preview").width()/$("#myImg").width());*/
			$("#myImg").css("width","640px");
			$("#myImg").css("height","880px");
		}
	}
	function updateType() {
	...
	$("#backward_page").click(function(){
		var zTree=$.fn.zTree.getZTreeObj("tree_index");
		var nodes=zTree.getSelectedNodes();
		var level2_tree=zTree.getNodesByParam("level","2");
		var tree_length=level2_tree.length;
		if(tree_length.length==0)
			return ;
		var index=nodes.length==0?1:level2_tree.indexOf(nodes[0]);
		index =index + (-1);
		if(index==-1)
			alert("当前为第一页！");
		else{
			var next_node=level2_tree[index];
			zTree.selectNode(next_node,false);
			$(".img_preview").html("").append('<img id="myImg" />');
			$("#myImg").prop("src",next_node.path);
			/*resizeImg($(".img_preview").width()/$("#myImg").width());*/
			$("#myImg").css("width","640px");
			$("#myImg").css("height","880px");
		}
	...
	$("#rotateBtn").on("click",function(){
		var img=$('#myImg');
		var angle=img.getRotateAngle()[0]==null?90 :(img.getRotateAngle()[0]+90);
		img.rotate(angle);
	});
	...
	$("#go_straight").click(function(){
		var pageNo=$.trim($("#page_num").val());
		if(pageNo==""){
			alert("请填写要跳转到的页面的页码！");
			$("#page_num").focus();
			return;
		...
		if(pageNo==0 || pageNo>tree_length){
			alert("正确的页码范围为1~"+tree_length+"，请重新输入!");
			$("#page_num").focus();
			return;
		}
		var next_node=level2_tree[pageNo-1];
		zTree.selectNode(next_node,false);
		$(".img_preview").html("").append('<img id="myImg" />');
		$("#myImg").prop("src",next_node.path);
		/*resizeImg($(".img_preview").width()/$("#myImg").width());*/
